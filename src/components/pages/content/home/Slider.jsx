...
function Slider({ topPlayList_t, list }) {
    ...
    const handleMove = (isLeft) => {
        ...
        interValFake.current = setInterval(() => {
            ...
            } else if (!isLeft && parseInt(sliderRef.current.style.left) > (-1790 + containerRef.current.offsetWidth)) {
                sliderRef.current.style.left = `${parseInt(sliderRef.current.style.left) - 10}px`
                sliderRef.current.style.right = `${parseInt(sliderRef.current.style.right) + 10}px`
            }
        ...
        <div className={styles['slider-container']}>
            <div className={styles['slider-header']}>
                ...
                <div className={styles['move-buttons']}>
                    <button className={`${styles['move-button']} left-arrow`} onMouseDown={() => handleMove(true)} onMouseUp={() => clearInterval(interValFake.current)}></button>
